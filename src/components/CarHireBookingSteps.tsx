      ...
      <div ref={containerRef} className="max-w-5xl mx-auto grid md:grid-cols-3 gap-6">
        {steps.map((step, index) => {
          ...
          return (
            <div
              key={step.title}
              className={`group relative rounded-[32px] shadow-sm hover:shadow-xl transition-all duration-700 ${
                visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'
              }`}
              style={{ transitionDelay: `${index * 140}ms`, perspective: '1200px' }}
            >
              <div
                className="relative h-full min-h-[280px] rounded-[32px] border border-gray-200 bg-gray-50 p-6 text-center transition-transform duration-[1200ms] ease-in-out"
                style={{
